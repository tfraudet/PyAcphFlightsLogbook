...
GROUND_SPEED_THRESHOLD	= 20		# 40km/h
...
class FlightsLogBookv2:
	...
	def handlePosition(self, beacon):
		...
		if self.isAircraftBeacon(beacon) and self.filteringReceivers(beacon['receiver_name']):
			# try:
				self.handleAircraftPosition(beacon)
	...
	def handleAircraftPosition(self, beacon):
		...
		lg_entry = self.findLogbookEntryByID(aircraft_id,beacon['timestamp'].strftime('%Y-%m-%d'), OGN_SENDER_TYPES[beacon['aircraft_type']], ognDevice)
		if (lg_entry is None):
